...
static std::string format_version_from_modbus(std::span<const uint8_t> data)
{
    if (data.size() >= 2)
    {
...
auto readSigned16 = [](std::span<const uint8_t> data, unsigned int offset) -> int16_t
{
...
// data: vettore contenente i dati Modbus raw
// offset: posizione di partenza nel vettore (in byte)
auto readUnsigned16 = [](std::span<const uint8_t> data, unsigned int offset) -> uint16_t
{
    int16_t raw = (int16_t)((data[offset] << 8) | data[offset + 1]);
    return raw;
};
...
// data: vettore contenente i dati Modbus raw
// offset: posizione di partenza nel vettore (in byte)
auto readUnsigned32 = [](std::span<const uint8_t> data, unsigned int offset) -> uint32_t
{
...
auto readSigned16ToFloat = [](std::span<const uint8_t> data, unsigned int offset, Scale scale) -> float
{
    int16_t raw = (int16_t)((data[offset] << 8) | data[offset + 1]);
...
auto readFloat = [](std::span<const uint8_t> data, unsigned int offset) -> float
{
    union
    {
        uint32_t i;
...
auto readBitFromUns16(std::span<const uint8_t> data, unsigned int offset, unsigned int bit_position, BitOrder order = BitOrder::LSB_FIRST) -> bool
{
...
auto readNBitsFromUns16(std::span<const uint8_t> data, unsigned int offset, unsigned int bit_position, unsigned int num_bits, BitOrder order = BitOrder::LSB_FIRST) -> uint8_t
{
    // Verifica che ci siano almeno 2 byte disponibili dall'offset specificato
    if (offset + 1 >= data.size())
    {
